...
void CACAPClient::_StoreAddress(CAddressBook* adbk, const CAddressList* addrs)
{
	...
	for(CAddressList::const_iterator iter = addrs->begin(); iter != addrs->end(); iter++)
	{
		...
		if (items.size())
		{
			...
			cdstring p;
			p.steal(buffer.str());
			*reinterpret_cast<uint32_t*>(const_cast<char*>(p.c_str())) = buflen - sizeof(uint32_t);
		...
		else if (items.size() == 2)
		{
			...
			uint32_t buflen1 = 0;
			buffer1.write(reinterpret_cast<const char*>(&buflen1), sizeof(uint32_t));
			buffer1 << temp;
			buffer1 << '\0';
			...
			cdstring p;
			p.steal(buffer1.str());
			*reinterpret_cast<uint32_t*>(const_cast<char*>(p.c_str())) = buflen1 - sizeof(uint32_t);
			INETSendString(p, eQueueBuffer);
			INETSendString(cSpace);
			items.erase(items.begin());
			...
			uint32_t buflen2 = 0;
			buffer2.write(reinterpret_cast<const char*>(&buflen2), sizeof(uint32_t));
			buffer2 << temp;
			buffer2 << '\0';
			buffer2 << items.front().second;
			buflen2 = buffer2.pcount();
			p.steal(buffer2.str());
			*reinterpret_cast<uint32_t*>(const_cast<char*>(p.c_str())) = buflen2 - sizeof(uint32_t);
			INETSendString(p, eQueueBuffer);
			INETSendString("))");
			INETSendString(cSpace);
		}
